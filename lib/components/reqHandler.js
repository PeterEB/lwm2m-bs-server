        ...
        setTimeout(function () {
            request.delete(bs, ip, port, '/', function (err, rsp) {
                if (err) {
                    ...
                    request.write(bs, ip, port, 'lwm2mSecurity/' + securityObjCount, bsSecurityObj, function (err, rsp) {
                        if (err) {
                            bs.emit('error', err);
                        } else if (rsp.status === RSP.changed) {
                            securityObjCount += 1;
